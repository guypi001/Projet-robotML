...
export class Robot implements Entities{
    ...
    move(dist: number): void {
        const deltaX = Math.cos(this.rad) * dist;
        const deltaY = Math.sin(this.rad) * dist;
        this.pos = this.pos.plus(new Vector(deltaX, deltaY));
    }

    side(dist: number): void {
        const sideRad = this.rad - Math.PI / 2;
        const deltaX = Math.cos(sideRad) * dist;
        const deltaY = Math.sin(sideRad) * dist;
        this.pos = this.pos.plus(new Vector(deltaX, deltaY));
    }
